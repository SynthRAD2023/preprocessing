...
def register(fixed, moving, parameter, output):
    ...
    elastixImageFilter.SetFixedImage(
        moving_image)  # due to FOV differences CT first registered to MR an inverted in the end
...
def generate_overview(input_path, ref_path, mask_path, output_path, title=''):
    ...
    ax[0][0].imshow(input_img[int(im_shape[0] / 2), :, ::-1], cmap='gray', aspect=asp_ax)
    ax[0][1].imshow(ref_img[int(im_shape[0] / 2), :, ::-1], cmap='gray', aspect=asp_ax)
    ...
    ax[1][0].imshow(input_img[::-1, :, int(im_shape[2] / 2)], cmap='gray',aspect=asp_sag)
    ax[1][1].imshow(ref_img[::-1, :, int(im_shape[2] / 2)], cmap='gray',aspect=asp_sag)
    ...
    ax[2][0].imshow(input_img[::-1, int(im_shape[1] / 2), :], cmap='gray', aspect=asp_cor)
    ax[2][1].imshow(ref_img[::-1, int(im_shape[1] / 2), :], cmap='gray', aspect=asp_cor)
    ax[2][2].imshow(mask_img[::-1, int(im_shape[1] / 2), :], cmap='gray', aspect=asp_cor)
...
if __name__ == "__main__":
    ...
    parser.add_argument('operation',
                        help='select operation to perform (register, convert, segment, mask_mr, mask_ct,resample, correct,overview, clean)')
